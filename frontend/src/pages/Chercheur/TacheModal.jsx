...
import { X, ClipboardList, Save, Loader2, Plus, Trash2, Tag, FileText, User } from 'lucide-react';
...
export default function TacheModal({ wp, membres, onClose, onRefresh }) {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axiosClient.post('/taches', { ...formData, livrables: livrablesAttendu });
            onRefresh();
            onClose();
        } catch (err) {
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="space-y-5">
                        <div>
                            ...
                            <input type="text" required placeholder="Décrivez l'action à mener..." value={formData.nom} 
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 flex items-center gap-1"><User size={10}/> Responsable</label>
                    ...
                    <div className="pt-6 border-t border-slate-100 space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <button type="button" onClick={() => setLivrablesAttendu([...livrablesAttendu, { titre: '', type: 'Rapport_Technique' }])} 
                                className="text-indigo-600 text-[10px] font-black hover:underline">+ AJOUTER LIVRABLE</button>
                        </div>
                        {livrablesAttendu.map((liv, idx) => (
                            <div key={idx} className="flex gap-3 items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                ...
                                <input type="text" placeholder="Titre du document..." required value={liv.titre}
                                    onChange={(e) => {
                                ...
                                <select value={liv.type} onChange={(e) => {
                                    ...
                                    className="bg-white border border-slate-200 rounded-xl text-[10px] font-black px-3 py-1 outline-none">
                                    {typesLivrables.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                                </select>
                                <button type="button" onClick={() => setLivrablesAttendu(livrablesAttendu.filter((_,i)=>i!==idx))} className="text-rose-400"><Trash2 size={16}/></button>
                            </div>
                        ))}
                    </div>

                    <button type="submit" disabled={loading}
                        className="w-full py-5 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest hover:bg-indigo-600 shadow-xl flex items-center justify-center gap-3">
