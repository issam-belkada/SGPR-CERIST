  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // --- OPTIMISATION ET FIX DES DATES ---
        const validTasks = resTasks.data
          .filter(t => t.date_fin) // On ne garde que les tâches avec une date
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-4 md:p-8 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "Mes Projets", value: stats?.projets_count || 0, icon: <FolderRoot />, color: "bg-indigo-500", trend: "Collaboration active" },
          { label: "Missions", value: stats?.taches_count || 0, icon: <CheckSquare />, color: "bg-amber-500", trend: `${stats?.taches_terminees || 0} terminées` },
          { label: "Livrables", value: stats?.livrables_count || 0, icon: <FileText />, color: "bg-emerald-500", trend: "Documents déposés" },
          { label: "Complétion", value: `${stats?.ratio_completion || 0}%`, icon: <Target />, color: "bg-rose-500", trend: "Objectifs atteints" },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
            ...
            <div className="space-y-4">
              {recentTasks.length > 0 ? recentTasks.slice(0, 5).map((tache) => {
                const dateObj = formatTaskDate(tache.date_fin);
                  ...
                  <div key={tache.id} className="group flex items-center gap-6 p-4 rounded-[2rem] hover:bg-slate-50/80 transition-all border border-transparent hover:border-slate-100">
                    <div className="flex flex-col items-center gap-1 min-w-[50px]">
                      <span className="text-[9px] font-black text-slate-400 uppercase">
                        {dateObj ? dateObj.toLocaleDateString('fr-FR', {month: 'short'}) : 'N/A'}
                      </span>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3">
                        <p className="text-[10px] text-slate-400 font-bold uppercase truncate max-w-[250px]">
                          {tache.work_package?.projet?.titre || 'Projet non spécifié'}
                        </p>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-400 font-bold text-[10px] uppercase">Aucune tâche avec échéance trouvée</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           ...
           <div className="bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden group shadow-2xl">
            ...
            <Link to="/chercheur/proposer-projet" className="w-full bg-white text-slate-900 py-4 rounded-2xl font-black text-[10px] uppercase text-center block tracking-widest hover:bg-indigo-500 hover:text-white transition-all">
              Soumettre un projet
            </Link>
          </div>
        </div>
